...
class SetupService {
  ...
  static Future<Map<String, dynamic>> setupDefaultAdmin() async {
    try {
      debugPrint('[SetupService] 🚀 Bắt đầu setup admin account...');
      ...
      debugPrint('[SetupService] 📋 Kiểm tra permission group Admin: ${adminPermQuery.docs.length} found');
      ...
      if (adminPermQuery.docs.isEmpty) {
        ...
        debugPrint('[SetupService] ✅ Tạo permission group Admin: $adminPermissionGroupId');
      } else {
        adminPermissionGroupId = adminPermQuery.docs.first.id;
        debugPrint('[SetupService] ℹ️  Permission group Admin đã tồn tại: $adminPermissionGroupId');
      ...
      debugPrint('[SetupService] 📋 Kiểm tra admin account: ${adminUserQuery.docs.length} found');
      ...
      if (adminUserQuery.docs.isEmpty) {
        ...
        try {
          ...
          debugPrint('[SetupService] ✅ Tạo Firebase Auth user: $adminEmail (uid: $uid)');
        } catch (e) {
          if (e.toString().contains('email-already-in-use')) {
            // Email already exists, try to find the user
            debugPrint('[SetupService] ℹ️  Email $adminEmail đã tồn tại, kiểm tra lại...');
            ...
            try {
              ...
              debugPrint('[SetupService] ✅ Firebase Auth user tồn tại: $adminEmail (uid: $uid)');
              
              // Sign out immediately after getting the UID
              await _firebaseAuth.signOut();
            } catch (signInError) {
              debugPrint('[SetupService] ⚠️  Không thể sign in: $signInError');
              return {
                'success': false,
        ...
        debugPrint('[SetupService] ✅ Tạo Firestore doc admin_accounts/$uid');
        ...
        debugPrint('[SetupService] ✅ Tạo Firestore doc users/$uid');
        ...
        debugPrint('[SetupService] ℹ️  Admin account đã tồn tại');
        return {
          'success': true,
  ...
  static Future<void> debugShowAllAccounts() async {
    try {
      debugPrint('[SetupService] 🔍 DEBUG: Tất cả tài khoản trong admin_accounts:');
      final adminAccounts = await _firestore.collection('admin_accounts').get();
      
      if (adminAccounts.docs.isEmpty) {
        debugPrint('[SetupService] ⚠️  Không có tài khoản nào');
      } else {
        for (var doc in adminAccounts.docs) {
          final data = doc.data();
          debugPrint('[SetupService] 📋 - uid: ${doc.id}, username: ${data['username']}, email: ${data['authEmail']}, isProtected: ${data['isProtected']}');
        }
      }
      
      debugPrint('[SetupService] 🔍 DEBUG: Tất cả permission groups:');
      final permissions = await _firestore.collection('permissions').get();
      
      if (permissions.docs.isEmpty) {
        debugPrint('[SetupService] ⚠️  Không có permission group nào');
      } else {
        for (var doc in permissions.docs) {
          final data = doc.data();
          debugPrint('[SetupService] 📋 - id: ${doc.id}, name: ${data['name']}, isProtected: ${data['isProtected']}');
        }
      }
    } catch (e) {
      debugPrint('[SetupService] ❌ Debug error: $e');
    }
  }
  ...
  static Future<void> _ensureAdminPermissionHasFullAccess(
    ...
    debugPrint('[SetupService] ✅ Đã đồng bộ quyền Admin đầy đủ cho group $permissionGroupId');
  }
  ...
  static Future<Map<String, dynamic>> cleanupFailedFirebaseUser(
    ...
    try {
      debugPrint('[SetupService] 🧹 Attempting to clean up failed user: $email');
      ...
      if (user != null) {
        // Delete the user
        await user.delete();
        debugPrint('[SetupService] ✅ Xóa Firebase user thành công: $email');
        return {
          'success': true,
      ...
      debugPrint('[SetupService] ❌ Cleanup error: $e');
      return {
        'success': false,
